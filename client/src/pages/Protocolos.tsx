...
import { FlaskConical, Pencil, Plus, Power, ShieldAlert, Trash2 } from "lucide-react";
...
export default function Protocolos() {
  ...
  const [protocoloEmEdicaoId, setProtocoloEmEdicaoId] = useState<number | null>(null);
  ...
  const actualizar = trpc.protocolos.actualizar.useMutation({
    onSuccess: () => {
      toast.success("Protocolo actualizado com sucesso.");
      utils.protocolos.list.invalidate();
      fecharModal();
    },
    onError: (erro) => toast.error(`Não foi possível actualizar o protocolo: ${erro.message}`),
  ...
  function fecharModal() {
    ...
    setProtocoloEmEdicaoId(null);
  }

  function abrirEdicao(protocolo: any) {
    setProtocoloEmEdicaoId(protocolo.id);
    setNome(protocolo.nome);
    setDescricao(protocolo.descricao ?? "");
    setTipoCuba(protocolo.tipoCuba);
    setEtapas(protocolo.etapas.length ? protocolo.etapas.map((etapa: any) => ({
      id: etapa.id,
      ordem: etapa.ordem,
      titulo: etapa.titulo,
      ...
      operador: etapa.operador ?? "menor_igual",
      valorGatilho: etapa.valorGatilho ?? "",
      produto: etapa.produto ?? "",
      dosePorHl: etapa.dosePorHl ?? "",
  ...
  function guardar() {
    ...
    if (protocoloEmEdicaoId) actualizar.mutate({ id: protocoloEmEdicaoId, ...dados });
    ...
    <div className="max-w-6xl mx-auto px-4 sm:px-6 py-6">
        ...
        {protocolos.map((protocolo) => (
          <article key={protocolo.id} className={`rounded-2xl border bg-white p-5 shadow-sm ${protocolo.ativo ? "border-gray-100" : "border-gray-200 opacity-65"}`}>
            <div className="flex justify-between gap-3">
              ...
              {canEdit && <div className="flex gap-2"><button onClick={() => abrirEdicao(protocolo)} className="h-8 rounded-lg border border-gray-200 px-2 text-xs font-medium text-gray-600 hover:bg-gray-50" title="Editar protocolo"><Pencil size={14} /></button><button onClick={() => definirEstado.mutate({ id: protocolo.id, ativo: !protocolo.ativo })} className="h-8 rounded-lg border border-gray-200 px-2 text-xs font-medium text-gray-600 hover:bg-gray-50" title={protocolo.ativo ? "Desactivar protocolo" : "Activar protocolo"}><Power size={14} /></button></div>}
            </div>
      ...
      <Dialog open={aberto} onOpenChange={(valor) => valor ? setAberto(true) : fecharModal()}>
        <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
          <DialogHeader><DialogTitle>{protocoloEmEdicaoId ? "Editar protocolo de fermentação" : "Novo protocolo de fermentação"}</DialogTitle></DialogHeader>
          ...
          <DialogFooter><button onClick={fecharModal} className="rounded-lg border border-gray-300 px-4 py-2 text-sm font-medium">Cancelar</button><button onClick={guardar} disabled={criar.isPending || actualizar.isPending} className="rounded-lg bg-[var(--color-vinho)] px-4 py-2 text-sm font-semibold text-white disabled:opacity-50">{criar.isPending || actualizar.isPending ? "A guardar..." : protocoloEmEdicaoId ? "Guardar alterações" : "Criar protocolo"}</button></DialogFooter>
        </DialogContent>
      </Dialog>
    </div>
  );
